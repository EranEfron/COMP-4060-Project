...
export default {
  ...
  methods: {
    ...
    async Login() {
      ...
      this.$cookies.set("token", "test", { expires: "1d" })
      ...
      if (apiResponse.data.success == true) {
        ...
        if (this.$cookies.get(this.ruleForm.name) == null) {
          let store_info = { name: this.ruleForm.name, identity: this.ruleForm.identity }
          this.$cookies.set(this.ruleForm.name, JSON.stringify(store_info), { expires: "1d" })
          console.log(this.$cookies.get(this.ruleForm.name));
        }
