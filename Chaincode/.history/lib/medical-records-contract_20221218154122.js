...
class MedicalRecordsContract extends Contract {
    ...
    async authorizeUser(ctx,owner_name,auth_name){
        const exists = await this.medicalRecordsExists(ctx, owner_name);
        if (!exists) {
    ...
    async registerUser(ctx, username, password, identity,id) {
        ...
        try {
            ...
            if (!exist) {
                ...
                returnMsg = { success: true, description: 'Registered successfully' ,id:id}
            }
        } catch (error) {
