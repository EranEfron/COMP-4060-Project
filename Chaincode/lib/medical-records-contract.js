...
class MedicalRecordsContract extends Contract {
    ...
    async createMedicalRecords(ctx, medicalRecordsId, uploadby) { // create a new patient with stored records
        ...
        privateAsset.uploadby = uploadby;
        const ownername = medicalRecordsId.split('\'')
        privateAsset.dataOwner = ownername[0]
        let currentTime = new Date();
        privateAsset.createdTime = currentTime;
        const collectionName = await getCollectionName(ctx);
    ...
    async updateMedicalRecords(ctx, medicalRecordsId, uploadby) {
        const exists = await this.medicalRecordsExists(ctx, medicalRecordsId);
        if (!exists) {
        ...
        privateAsset.uploadby = uploadby;
        privateAsset.dataOwner = privateData.dataOwner;
        privateAsset.createdTime = privateData.createdTime;
        await ctx.stub.putPrivateData(collectionName, medicalRecordsId, Buffer.from(JSON.stringify(privateAsset)));
    ...
    async queryRecords(ctx, recordName, requestor) {
        const exists = await this.medicalRecordsExists(ctx, recordName);
        if (!exists) {
            return { success: false, description: `The asset medical records ${recordName} does not exist` }// throw new Error(`The asset medical records ${medicalRecordsId} does not exist`);
        } else {
            const collectionName = await getCollectionName(ctx);
            const privateData = await ctx.stub.getPrivateData(collectionName, recordName);
            let result = JSON.parse(privateData.toString());
            let auth_list = result.Authorized_list;
            let userAuthorized = false;
            if (requestor == result.dataOwner) {
                userAuthorized = true;
            } else {
                for (let i = 0; i < auth_list.length; i++) {
                    if (auth_list[i].name == requestor) {
                        userAuthorized = true;
                    }
                }
            }
            if (userAuthorized) {
                return { success: true, description: `{"recordName":"${recordName}", "timeStamp":"${result.createdTime}","uploadby":"${result.uploadby}","dataOwner":"${result.dataOwner}"}` }
            }else{
                return { success: false, description: `You are not authorized to access this record` }
            }
        }
    }
