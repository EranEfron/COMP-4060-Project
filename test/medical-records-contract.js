...
describe('MedicalRecordsContract', () => {
    ...
    describe('#medicalRecordsExists', () => {
        ...
        it('should return true for a private asset that exists', async () => {
            await contract.medicalRecordsExists(ctx, '001').should.eventually.be.true;
        });

        it('should return false for a private asset that does not exist', async () => {
            await contract.medicalRecordsExists(ctx, '002').should.eventually.be.false;
        });
    ...
    describe('#createMedicalRecords', () => {
        ...
        it('should throw an error for a private asset that already exists', async () => {
            await contract.createMedicalRecords(ctx, '001').should.be.rejectedWith('The asset medical records 001 already exists');
        });
        ...
        it('should throw an error if transient data is not provided when creating private asset', async () => {
            ...
            await contract.createMedicalRecords(ctx, '002').should.be.rejectedWith('The privateValue key was not specified in transient data. Please try again.');
        });
        ...
        it('should create a private asset if transient data key is privateValue', async () => {
            ...
            ctx.stub.getTransient.returns(transientMap);
            await contract.createMedicalRecords(ctx, '002');
    ...
    describe('#readMedicalRecords', () => {
        ...
        it('should throw an error for my private asset that does not exist', async () => {
            await contract.readMedicalRecords(ctx, '003').should.be.rejectedWith('The asset medical records 003 does not exist');
        });

        it('should return my private asset', async () => {
            await contract.readMedicalRecords(ctx, '001').should.eventually.deep.equal({ privateValue: '150' });
    ...
    describe('#updateMedicalRecords', () => {
        ...
        it('should throw an error for my private asset that does not exist', async () => {
            await contract.updateMedicalRecords(ctx, '003').should.be.rejectedWith('The asset medical records 003 does not exist');
        });

        it('should throw an error if transient data is not provided when updating private asset', async () => {
            let transientMap = new Map();
            ctx.stub.getTransient.returns(transientMap);
            await contract.updateMedicalRecords(ctx, '001').should.be.rejectedWith('The privateValue key was not specified in transient data. Please try again.');
        });
        ...
        it('should update my private asset if transient data key is privateValue', async () => {
            ...
            ctx.stub.getTransient.returns(transientMap);
            await contract.updateMedicalRecords(ctx, '001');
    ...
    describe('#deleteMedicalRecords', () => {
        ...
        it('should throw an error for my private asset that does not exist', async () => {
            await contract.deleteMedicalRecords(ctx, '003').should.be.rejectedWith('The asset medical records 003 does not exist');
        });

        it('should delete my private asset', async () => {
            await contract.deleteMedicalRecords(ctx, '001');
            ctx.stub.deletePrivateData.should.have.been.calledOnceWithExactly(collectionName, '001');
        });

    });

    describe('#verifyMedicalRecords', () => {
        ...
        it('should return success message if hash provided matches the hash of the private data', async () => {
            ...
            ctx.stub.getPrivateDataHash.withArgs(collectionName, '001').resolves(Buffer.from(hashToVerify, 'hex'));
            const result = await contract.verifyMedicalRecords(ctx, mspid, '001', '{"privateValue":"125"}');
        ...
        it('should throw an error if hash provided does not match the hash of the private data', async () => {
            ...
            const result = await contract.verifyMedicalRecords(ctx, mspid, '001', 'someObject');
        ...
        it('should throw an error when user tries to verify an asset that doesnt exist', async () => {
            ...
            await contract.verifyMedicalRecords(ctx, mspid, '001', 'someObject').should.be.rejectedWith('No private data hash with the key: 001');
        });
    });
});
